...
const CartDetail = ({state}) => {
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      const response = await fetch(`${server}/api/cart/buynft`, {
        method: 'POST',
        headers: {
          'auth_token': token,
          'Content-Type': 'application/json',
 ...
 // Function to remove NFT from cart
   const removeFromCart = async (imgpath) => {
     const token = localStorage.getItem('token');
     ...
     try {
       const response = await axios.delete(`${server}/api/cart/deletebyimgpath`, {
         headers: {
           auth_token: token,
           'Content-Type': 'application/json'
         },
       ...
       if (response.data.success) {
         ...
         setNfts(nfts.filter(nft => nft.imgpath !== imgpath));
         navigate('/cart')
       } else {
         alert('Failed to remove NFT from cart');
       }
     } catch (error) {
       console.error('Error removing NFT from cart:', error);
       alert('Error removing NFT from cart');
     }
   };
          ...
          <div id="carder">
            ...
            <div id="innercarder">
              ...
              <button
                    ...
                    onClick={() => removeFromCart(nft.imgpath)}
